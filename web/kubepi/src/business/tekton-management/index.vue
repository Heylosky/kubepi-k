<template>
  <layout-content :header="$t('business.tekton.list')">
    ...
    <complex-table v-loading="loading" :search-config="searchConfig" :selects.sync="selects" :data="data"
      ...
      <el-table-column :label="$t('commons.table.status')" min-width="80px" fix>
        <template v-slot:default="{row}">
          <el-tag type="success" v-if="row.extraClusterInfo.health">{{ $t('business.tekton.ready') }}</el-tag>
          <el-tooltip class="item" effect="dark" :content="row.extraClusterInfo.message" placement="right">
            <el-tag type="danger" v-if="!row.extraClusterInfo.health">{{ $t('business.tekton.not_ready') }}</el-tag>
          </el-tooltip>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('business.tekton.label')" align="left" min-width="160" fix>
        <template v-slot:default="{row}">
          <div style="font-size: 20px">
            ...
            <div style="padding-top: 5px;">
              <el-popover
                placement="top"
                width="160"
                :title="$t('commons.button.add')+$t('business.tekton.label')"
                trigger="manual"
      ...
      <el-table-column :label="$t('business.tekton.version')" min-width="100" fix>
        <template v-slot:default="{row}">
          <el-tag>
            {{ row.status.version }}
          </el-tag>
    ...
    return {
      ...
      searchConfig: {
        ...
        components: [
          ...
          {
            field: "labels",
            label: this.$t("business.tekton.label"),
  ...
  methods: {
    ...
    onGotoDashboard(row) {
      if (row.accessable) {
        ...
        const url = `/apis/cluster.karmada.io/v1alpha1/clusters/${row.name}/proxy/api/v1/namespaces/tekton-pipelines/services/tekton-dashboard:9097/proxy/`
        window.open(url, "_blank")
      } else {
        this.$message.error(this.$t('business.tekton.user_not_in_cluster'))
      }
    },
    checkPrem(obj) {
